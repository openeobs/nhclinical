...
class TestResponsibilityAllocationWizard(SingleTransactionCase):
    ...
    def test_02_onchange_user_id_returns_search_parameter_dict(self):
        ...
        result = self.wizard_pool.onchange_user_id(cr, uid, None, self.adt_uid, context=None)
        self.assertEquals(result, {'value': {'location_ids': [[6, False, [self.hospital_id]]]}})
    ...
    def test_04_onchange_clear_does_not_append_clear_location_when_clear_is_False(self):
        cr, uid = self.cr, self.uid
        clears = [False, None, '']
    ...
    def test_06_get_location_list_with_nonexistent_location_id_raises_error(self):
        cr, uid = self.cr, self.uid

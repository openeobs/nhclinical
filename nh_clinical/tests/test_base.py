...
class BaseTest(SingleTransactionCase):
    ...
    def setUp(self):
        global cr, uid, seed
        global patient_pool, location_pool, pos_pool, user_pool, imd_pool
        global activity_pool, device_type_pool, device_pool
        global pos_id, ward_location_ids, bed_location_ids, adt_user_id
        global nurse_user_ids
    ...
    def create_activity(self, cr, uid, data_model, vals_activity={},
    ...
    def create_pos_environment(self, WARD_QTY=5, BED_PER_WARD=3, DEVICES=5):
        ...
        # Create wards
        ward_location_ids = [
            self.create_ward(pos.location_id.id) for i in range(WARD_QTY)]

        # Create beds
        bed_location_ids = []
        for ward_location_id in ward_location_ids:
            bed_location_ids.extend(
                [self.create_bed(ward_location_id) for i in range(BED_PER_WARD)
                 ])
    ...
    def create_admission_location(self, parent_id=None):
        data = self.data_admission_location(
            parent_id and {'parent_id': parent_id} or {})
        location_id = location_pool.create(cr, uid, data)
    ...
    def create_discharge_location(self, parent_id=None):
        data = self.data_discharge_location(
            parent_id and {'parent_id': parent_id} or {})
        location_id = location_pool.create(cr, uid, data)
    ...
    def data_patient(self, data={}):
        ...
        other_identifier = data.get('other_identifier') or str(
            fake.random_int(min=1000001, max=9999999))
        dob = data.get('dob') or fake.date_time_between(
            start_date="-80y", end_date="-10y").strftime("%Y-%m-%d %H:%M:%S")
        res = {
    ...
    def data_adt_user(self, data={}):
        ...
        res = {
            ...
            'login': data.get('login') or "adt_%s" %
                                          pos.location_id.code.lower(),
            'password': data.get('password') or "adt_%s" %
                                                pos.location_id.code.lower(),
    ...
    def data_nurse_user(self, location_ids, data={}):
        ...
        res = {
            ...
            'groups_id': data.get('groups_id') and
            [(4, group_id) for group_id in data['groups_id']] or
            [(4, nurse_group.id)],
    ...
    def data_pos(self, data={}):
        ...
        res = {
            ...
            'lot_admission_id': data.get('lot_admission_id') or
            self.create_admission_location(location_id),
            'lot_discharge_id': data.get('lot_discharge_id') or
            self.create_discharge_location(location_id)
        }
        return res
    ...
    def data_device_type(self, data={}):
        ...
        res = {
            ...
            "DEVICE_TYPE_"+str(fake.random_int(min=100, max=999)),
            'flow_direction': data.get('flow_direction') or
            flow_directions[fake.random_int(min=0, max=3)],
        }
        return res

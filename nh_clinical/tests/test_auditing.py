...
class TestAuditing(common.SingleTransactionCase):
    ...
    def test_user_responsibility_allocation(self):
        ...
        self.assertTrue(user.location_ids, msg="Scenario 1 Pre-state: User is not responsible for any locations")
        activity_id = self.allocation_pool.create_activity(cr, uid, {}, {
            'responsible_user_id': self.nu_id,
            'location_ids': [[6, False, []]]})
        self.assertTrue(activity_id, msg="User Responsibility Allocation Activity creation failed")
        activity = self.activity_pool.browse(cr, uid, activity_id)
        self.assertEqual(activity.data_ref.responsible_user_id.id, self.nu_id, msg="User Responsibility Allocation Activity: wrong user recorded")
        self.assertFalse(activity.data_ref.location_ids, msg="User Responsibility Allocation Activity: wrong locations recorded")
        self.activity_pool.complete(cr, uid, activity_id)
        user = self.users_pool.browse(cr, uid, self.nu_id)
        self.assertFalse(user.location_ids, msg="Scenario 1 Failed: User is still responsible for some locations")

        # Scenario 2: Assigning a bed to a nurse
        bed_ids = self.location_pool.search(cr, uid, [['usage', '=', 'bed'], ['id', 'child_of', self.wu_id]])
        activity_id = self.allocation_pool.create_activity(cr, uid, {}, {
            'responsible_user_id': self.nu_id,
            'location_ids': [[6, False, [bed_ids[0]]]]})
        self.activity_pool.complete(cr, uid, activity_id)
        user = self.users_pool.browse(cr, uid, self.nu_id)
        self.assertEqual(bed_ids[0], user.location_ids.id, msg="Scenario 2 Failed: User locations don't match")

        # Scenario 3: Assigning a ward to a nurse
        location_ids = self.location_pool.search(cr, uid, [['id', 'child_of', self.wu_id]])
        activity_id = self.allocation_pool.create_activity(cr, uid, {}, {
            'responsible_user_id': self.nu_id,
            'location_ids': [[6, False, [self.wu_id]]]})
        self.activity_pool.complete(cr, uid, activity_id)
        user = self.users_pool.browse(cr, uid, self.nu_id)
        self.assertEqual(location_ids, [loc.id for loc in user.location_ids], msg="Scenario 3 Failed: User locations don't match")

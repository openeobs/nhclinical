...
class TestUserAllocation(common.SingleTransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.admin_group_id = cls.group_pool.search(cr, uid, [['name', '=', 'NH Clinical Admin Group']])[0]
        ...
        cls.adt_uid = cls.user_pool.create(cr, uid, {'name': 'Test User', 'login': 'user_001',
                                                     'password': 'user_001', 'groups_id': [[4, cls.admin_group_id]],
                                                     'pos_id': cls.pos_id})
    ...
    def test_01_allocation_fields_view_get(self):
        ...
        res = self.allocation_pool.fields_view_get(cr, uid, view_id=None, view_type='form', context=None,
        ...
        res = self.allocation_pool.fields_view_get(cr, uid, view_id=None, view_type='tree', context=None,
    ...
    def test_02_allocation_submit_users(self):
        ...
        allocation_id = self.allocation_pool.create(cr, self.adt_uid, {
            ...
            'user_ids': [[6, 0, [self.hca_uid, self.nurse_uid, self.wm_uid, self.sm_uid, self.dr_uid]]]
        })
        ...
        for allocating in allocation.allocating_user_ids:
            self.assertTrue(allocating.user_id.id in [self.hca_uid, self.nurse_uid,
                                                      self.wm_uid, self.sm_uid, self.dr_uid], msg="Wrong User ID")
            self.assertFalse(allocating.location_ids, msg="Location list should be empty")

    def test_03_allocating_get_roles(self):
        cr, uid = self.cr, self.uid

        # Scenario 1: Get Clinical roles for every user type
        allocation_id = self.allocation_pool.search(cr, uid, [['create_uid', '=', self.adt_uid]])
        allocation = self.allocation_pool.browse(cr, uid, allocation_id[0])
        for allocating_user in allocation.allocating_user_ids:
            res = self.allocating_pool._get_roles(cr, uid, [allocating_user.id], ['roles'], None)
    ...
    def test_04_allocating_fields_view_get(self):
        ...
        res = self.allocating_pool.fields_view_get(cr, uid, view_id=None, view_type='form', context=None,
                                                   toolbar=False, submenu=False)
        self.assertTrue(res, msg="No result returned")

        # Scenario 2: Get Form view fields for user with allocation wizard
        res = self.allocating_pool.fields_view_get(cr, self.adt_uid, view_id=None, view_type='form', context=None,
                                                   toolbar=False, submenu=False)
        self.assertListEqual(res['fields']['location_ids']['domain'][0], ['id', 'in', [self.bed_id]])

        # Scenario 3: Get non Form view fields
        res = self.allocating_pool.fields_view_get(cr, uid, view_id=None, view_type='tree', context=None,
                                                   toolbar=False, submenu=False)
        self.assertTrue(res, msg="No result returned")
    ...
    def test_05_allocation_complete(self):
        ...
        for allocating_user in allocation.allocating_user_ids:
            if allocating_user.user_id.id in [self.hca_uid, self.nurse_uid]:
                self.allocating_pool.write(cr, self.adt_uid, allocating_user.id, {
        ...
        for allocating_user in allocation.allocating_user_ids:
            resp_id = self.resp_pool.search(cr, uid, [['responsible_user_id', '=', allocating_user.user_id.id]])
            self.assertTrue(resp_id, msg="Missing Responsibility Allocation activity")

...
class TestAllocationWizards(SingleTransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.admin_uid = cls.users_pool.create(cr, uid, {'name': 'Admin 0', 'login': 'user_000',
                                                        'password': 'user_000',
        ...
        for i in range(3):
            ...
            cls.locations[wid] = [cls.location_pool.create(cr, uid, {'name': 'Bed'+str(i)+str(j),
                                                                     ...
                                                                     'type': 'poc'}) for j in range(10)]
            cls.users['wm'].append(cls.users_pool.create(cr, uid, {'name': 'Ward Manager '+str(i), 'login': 'WM'+str(i),
                                                                   'password': 'WM'+str(i),
                                                                   'groups_id': [[4, cls.wm_group_id[0]]],
                                                                   'pos_id': cls.pos_id}))
            cls.users['ns'].append(cls.users_pool.create(cr, uid, {'name': 'Nurse '+str(i), 'login': 'NURSE'+str(i),
                                                                   'password': 'NURSE'+str(i),
                                                                   'groups_id': [[4, cls.nurse_group_id[0]]],
                                                                   'pos_id': cls.pos_id}))
            cls.users['hc'].append(cls.users_pool.create(cr, uid, {'name': 'HCA '+str(i), 'login': 'HCA'+str(i),
                                                                   'password': 'HCA'+str(i),
                                                                   'groups_id': [[4, cls.hca_group_id[0]]],
                                                                   'pos_id': cls.pos_id}))
            cls.users['dr'].append(cls.users_pool.create(cr, uid, {'name': 'Doctor '+str(i), 'login': 'DR'+str(i),
                                                                   'password': 'DR'+str(i),
    ...
    def test_01_nursing_staff_allocation(self):
        ...
        self.assertTrue(ward_id in location_ids, msg="Ward ID missing from location ids")
        for bed_id in self.locations[ward_id]:
            self.assertTrue(bed_id in location_ids, msg="Bed ID missing from location ids")
        ...
        self.assertEqual(self.users_pool.read(cr, uid, wm_uid, ['location_ids'])['location_ids'], [ward_id])
        ...
        for alid in allocating_ids:
            self.assertTrue(self.allocating.read(cr, uid, alid, ['location_id'])['location_id'][0] in location_ids)

        # Step 4: Selecting the Nursing Staff
        user_ids = [self.users['ns'][0], self.users['ns'][1], self.users['hc'][0]]
        self.staff_allocation.write(cr, wm_uid, sawiz_id, {'user_ids': [[6, 0, user_ids]]})
        self.staff_allocation.submit_users(cr, wm_uid, sawiz_id)
        self.assertEqual(self.staff_allocation.read(cr, uid, sawiz_id, ['stage'])['stage'], 'allocation')
        ...
        nurse_ids = self.users_pool.search(cr, uid, view_fields['fields']['nurse_id']['domain'])
        hca_ids = self.users_pool.search(cr, uid, view_fields['fields']['hca_id']['domain'])
        ...
        alternate = True
        for alid in allocating_ids:
            self.allocating.write(cr, wm_uid, alid, {'nurse_id': user_ids[0] if alternate else user_ids[1],
                                                     'hca_id': user_ids[2]})
            alternate = not alternate

        # Step 6: Complete Allocation
        self.staff_allocation.complete(cr, wm_uid, sawiz_id)
        nurse1_loc_ids = self.users_pool.read(cr, uid, user_ids[0], ['location_ids'])['location_ids']
        nurse2_loc_ids = self.users_pool.read(cr, uid, user_ids[1], ['location_ids'])['location_ids']
        hca_loc_ids = self.users_pool.read(cr, uid, user_ids[2], ['location_ids'])['location_ids']
        ...
        for loc_id in nurse1_loc_ids:
            self.assertTrue(loc_id in location_ids, msg="User assigned to wrong location")
    ...
    def test_02_nursing_staff_reallocation(self):
        ...
        self.assertEqual(self.staff_reallocation.read(cr, uid, srwiz_id, ['stage'])['stage'], 'users')
        self.assertEqual(self.staff_reallocation.read(cr, uid, srwiz_id, ['ward_id'])['ward_id'][0], ward_id)
        location_ids = self.staff_reallocation.read(cr, uid, srwiz_id, ['location_ids'])['location_ids']
        self.assertTrue(ward_id in location_ids, msg="Ward ID missing from location ids")
        for bed_id in self.locations[ward_id]:
            self.assertTrue(bed_id in location_ids, msg="Bed ID missing from location ids")
        user_ids = self.staff_reallocation.read(cr, uid, srwiz_id, ['user_ids'])['user_ids']
        for u_id in user_ids:
            self.assertTrue(u_id in [self.users['ns'][0], self.users['ns'][1], self.users['hc'][0]])
        allocating_ids = self.staff_reallocation.read(cr, uid, srwiz_id, ['allocating_ids'])['allocating_ids']
        self.assertEqual(len(allocating_ids), len(location_ids)-1)
        for alid in allocating_ids:
            self.assertTrue(self.allocating.read(cr, uid, alid, ['location_id'])['location_id'][0] in location_ids)
            self.assertTrue(self.allocating.read(cr, uid, alid, ['nurse_id'])['nurse_id'][0] in user_ids)
            self.assertTrue(self.allocating.read(cr, uid, alid, ['hca_id'])['hca_id'][0] in user_ids)

        # Step 2: Re-Selecting Nursing Staff
        user_ids = [self.users['ns'][0], self.users['ns'][2], self.users['hc'][0]]
        self.staff_reallocation.write(cr, wm_uid, srwiz_id, {'user_ids': [[6, 0, user_ids]]})
        self.staff_reallocation.reallocate(cr, wm_uid, srwiz_id)
        allocating_ids = self.staff_reallocation.read(cr, uid, srwiz_id, ['allocating_ids'])['allocating_ids']
        self.assertEqual(self.staff_reallocation.read(cr, uid, srwiz_id, ['stage'])['stage'], 'allocation')
        for alid in allocating_ids:
            nurse_id = self.allocating.read(cr, uid, alid, ['nurse_id'])['nurse_id']
        ...
        for a in self.allocating.browse(cr, uid, allocating_ids):
            if not a.nurse_id:
                self.allocating.write(cr, wm_uid, a.id, {'nurse_id': user_ids[1]})
    ...
    def test_04_medical_staff_allocation(self):
        ...
        self.assertEqual(self.doctor_allocation.read(cr, uid, dawiz_id, ['stage'])['stage'], 'review')
        self.assertEqual(self.doctor_allocation.read(cr, uid, dawiz_id, ['ward_id'])['ward_id'][0], ward_id)
        location_ids = self.doctor_allocation.read(cr, uid, dawiz_id, ['location_ids'])['location_ids']
        self.assertTrue(ward_id in location_ids, msg="Ward ID missing from location ids")
        for bed_id in self.locations[ward_id]:
            self.assertTrue(bed_id in location_ids, msg="Bed ID missing from location ids")
        doctor_ids = self.doctor_allocation.read(cr, uid, dawiz_id, ['doctor_ids'])['doctor_ids']
        self.assertFalse(doctor_ids, msg="Found medical staff assigned to the Ward!")

        # Step 2: De-allocation
        self.doctor_allocation.deallocate(cr, wm_uid, dawiz_id)
        self.assertEqual(self.doctor_allocation.read(cr, uid, dawiz_id, ['stage'])['stage'], 'users')
        self.assertEqual(len(self.users_pool.search(cr, uid, [
            ['location_ids', 'in', location_ids], ['groups_id.name', 'in', ['NH Clinical Doctor Group']]])), 0)

        # Step 3: Selecting the Medical Staff
        user_ids = [self.users['dr'][0], self.users['dr'][1]]
        self.doctor_allocation.write(cr, wm_uid, dawiz_id, {'user_ids': [[6, 0, user_ids]]})
        self.doctor_allocation.submit_users(cr, wm_uid, dawiz_id)
        doctor1_loc_ids = self.users_pool.read(cr, uid, user_ids[0], ['location_ids'])['location_ids']
        doctor2_loc_ids = self.users_pool.read(cr, uid, user_ids[0], ['location_ids'])['location_ids']
        self.assertEqual(len(doctor1_loc_ids), 11)
        self.assertEqual(len(doctor2_loc_ids), 11)
    ...
    def test_05_medical_staff_new_shift_allocation(self):
        ...
        self.doctor_allocation.write(cr, wm_uid, dawiz_id, {'user_ids': [[6, 0, user_ids]]})
        self.doctor_allocation.submit_users(cr, wm_uid, dawiz_id)
        doctor_loc_ids = self.users_pool.read(cr, uid, user_ids[0], ['location_ids'])['location_ids']
        self.assertEqual(len(doctor_loc_ids), 11)
